...
Citizen.CreateThread(function()
    while true do
        ...
        if createdCamera ~= 0 then
            ...
            ---------------------------------------------------------------------------
            if SecurityCameras[currentCameraIndexIndex].canRotate then
...
RegisterNetEvent('cam:client:ActiveCamera')
AddEventHandler('cam:client:ActiveCamera', function(cameraId)
    if SecurityCameras[cameraId] ~= nil then
        DoScreenFadeOut(250)
        while not IsScreenFadedOut() do
            Citizen.Wait(0)
        end
        SendNUIMessage({
            type = "enablecam",
            label = SecurityCameras[cameraId].label,
            id = cameraId,
            connected = SecurityCameras[cameraId].isOnline,
            time = GetCurrentTime()
        })
        local firstCamx = SecurityCameras[cameraId].x
        local firstCamy = SecurityCameras[cameraId].y
        local firstCamz = SecurityCameras[cameraId].z
        local firstCamr = SecurityCameras[cameraId].r
...
RegisterNetEvent('cam:client:DisableAllCameras')
AddEventHandler('cam:client:DisableAllCameras', function()
    for k, v in pairs(SecurityCameras) do
        SecurityCameras[k].isOnline = false
    end
end)

RegisterNetEvent('cam:client:EnableAllCameras')
AddEventHandler('cam:client:EnableAllCameras', function()
    for k, v in pairs(SecurityCameras) do
        SecurityCameras[k].isOnline = true
    end
end)

RegisterNetEvent('cam:client:SetCamera')
AddEventHandler('cam:client:SetCamera', function(key, isOnline)
    SecurityCameras[key].isOnline = isOnline
end)
